...
CREATE TABLE wcdk_process_definition (
    ...
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    key                     VARCHAR2(128)   NOT NULL,           -- 流程定义Key
    name                    VARCHAR2(256),                      -- 流程定义名称
    version                 NUMBER(10)      NOT NULL DEFAULT 1, -- 版本号
    category                VARCHAR2(128),                      -- 流程分类
    description             VARCHAR2(1024),                     -- 流程描述
    deployment_id           VARCHAR2(64),                       -- 部署ID
    resource_name           VARCHAR2(256),                      -- 资源名称
    diagram_resource_name   VARCHAR2(256),                      -- 流程图资源名称
    graph_json              CLOB,                               -- 流程图JSON（缓存）
    suspended               NUMBER(1)       NOT NULL DEFAULT 0, -- 是否挂起（0-正常 1-挂起）
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
COMMENT ON COLUMN wcdk_process_definition.suspended IS '是否挂起（0-正常 1-挂起）';
...
CREATE TABLE wcdk_deployment (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    name                    VARCHAR2(256),                      -- 部署名称
    category                VARCHAR2(128),                      -- 部署分类
    deployment_time         TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 部署时间
    source_system           VARCHAR2(128),                      -- 来源系统
    description             VARCHAR2(1024),                     -- 部署描述
    version                 NUMBER(10)      NOT NULL DEFAULT 1, -- 版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_resource (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    deployment_id           VARCHAR2(64)    NOT NULL,           -- 部署ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    name                    VARCHAR2(256)   NOT NULL,           -- 资源名称
    resource_type           VARCHAR2(32)    NOT NULL,           -- 资源类型（BPMN/XML/JSON）
    content                 CLOB,                               -- 文本内容
    content_bytes           BLOB,                               -- 二进制内容
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
...
CREATE TABLE wcdk_process_instance (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    process_definition_id   VARCHAR2(64)    NOT NULL,           -- 流程定义ID
    process_definition_key  VARCHAR2(128)   NOT NULL,           -- 流程定义Key
    process_definition_version NUMBER(10)   NOT NULL,           -- 流程定义版本
    business_key            VARCHAR2(256),                      -- 业务KEY
    parent_process_instance_id VARCHAR2(64),                    -- 父流程实例ID
    root_process_instance_id VARCHAR2(64),                      -- 根流程实例ID
    starter                 VARCHAR2(128),                      -- 发起人
    start_time              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 开始时间
    end_time                TIMESTAMP,                          -- 结束时间
    duration_ms             NUMBER(19),                         -- 执行时长（毫秒）
    status                  VARCHAR2(32)    NOT NULL DEFAULT 'RUNNING', -- 状态（RUNNING/COMPLETED/TERMINATED/SUSPENDED）
    suspension_state        NUMBER(10)      NOT NULL DEFAULT 1, -- 挂起状态（1-激活 2-挂起）
    error_message           CLOB,                               -- 错误信息
    revision                NUMBER(19)      NOT NULL DEFAULT 1, -- 乐观锁版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_execution (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    process_instance_id     VARCHAR2(64)    NOT NULL,           -- 流程实例ID
    process_definition_id   VARCHAR2(64)    NOT NULL,           -- 流程定义ID
    parent_id               VARCHAR2(64),                       -- 父执行实例ID
    scope_execution_id      VARCHAR2(64),                       -- 作用域执行实例ID
    root_execution_id       VARCHAR2(64),                       -- 根执行实例ID
    node_id                 VARCHAR2(128)   NOT NULL,           -- 节点ID
    node_type               VARCHAR2(64)    NOT NULL,           -- 节点类型
    state                   VARCHAR2(32)    NOT NULL DEFAULT 'ACTIVE', -- 状态（ACTIVE/WAITING/COMPLETED/TERMINATED）
    is_scope                NUMBER(1)       NOT NULL DEFAULT 0, -- 是否为作用域（0-否 1-是）
    is_concurrent           NUMBER(1)       NOT NULL DEFAULT 0, -- 是否并发（0-否 1-是）
    is_event_scope          NUMBER(1)       NOT NULL DEFAULT 0, -- 是否为事件作用域（0-否 1-是）
    is_multi_instance       NUMBER(1)       NOT NULL DEFAULT 0, -- 是否多实例（0-否 1-是）
    multi_instance_index    NUMBER(10),                         -- 多实例索引
    multi_instance_total    NUMBER(10),                         -- 多实例总数
    suspension_state        NUMBER(10)      NOT NULL DEFAULT 1, -- 挂起状态（1-激活 2-挂起）
    revision                NUMBER(19)      NOT NULL DEFAULT 1, -- 乐观锁版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_task (
    ...
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    process_instance_id     VARCHAR2(64)    NOT NULL,           -- 流程实例ID
    execution_id            VARCHAR2(64)    NOT NULL,           -- 执行实例ID
    process_definition_id   VARCHAR2(64)    NOT NULL,           -- 流程定义ID
    task_definition_key     VARCHAR2(128),                      -- 任务定义Key
    name                    VARCHAR2(256),                      -- 任务名称
    state                   VARCHAR2(32)    NOT NULL DEFAULT 'CREATED', -- 状态（CREATED/CLAIMED/COMPLETED/CANCELLED）
    assignee                VARCHAR2(128),                      -- 处理人
    owner                   VARCHAR2(128),                      -- 拥有者
    priority                NUMBER(10)      NOT NULL DEFAULT 50, -- 优先级（0-100）
    due_time                TIMESTAMP,                          -- 到期时间
    create_time             TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    claim_time              TIMESTAMP,                          -- 认领时间
    complete_time           TIMESTAMP,                          -- 完成时间
    form_data               CLOB,                               -- 表单数据（JSON）
    description             VARCHAR2(1024),                     -- 任务描述
    revision                NUMBER(19)      NOT NULL DEFAULT 1, -- 乐观锁版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_identity_link (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    task_id                 VARCHAR2(64)    NOT NULL,           -- 任务ID
    process_instance_id     VARCHAR2(64)    NOT NULL,           -- 流程实例ID
    link_type               VARCHAR2(32)    NOT NULL,           -- 关系类型（ASSIGNEE/CANDIDATE/OWNER/NOTIFY）
    user_id                 VARCHAR2(128),                      -- 用户ID
    group_id                VARCHAR2(128),                      -- 用户组ID
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
...
CREATE TABLE wcdk_variable (
    ...
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    process_instance_id     VARCHAR2(64)    NOT NULL,           -- 流程实例ID
    scope_type              VARCHAR2(32)    NOT NULL,           -- 作用域类型（PROCESS/EXECUTION/TASK）
    scope_id                VARCHAR2(64)    NOT NULL,           -- 作用域ID
    name                    VARCHAR2(128)   NOT NULL,           -- 变量名
    value_type              VARCHAR2(32)    NOT NULL,           -- 值类型（STRING/LONG/DOUBLE/BOOLEAN/JSON/NULL）
    value_text              CLOB,                               -- 文本值
    value_long              NUMBER(19),                         -- 长整型值
    value_double            NUMBER,                             -- 浮点型值
    value_boolean           NUMBER(1),                          -- 布尔型值（0-否 1-是）
    revision                NUMBER(19)      NOT NULL DEFAULT 1, -- 乐观锁版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_event_subscription (
    ...
    process_instance_id     VARCHAR2(64)    NOT NULL,           -- 流程实例ID
    execution_id            VARCHAR2(64)    NOT NULL,           -- 执行实例ID
    event_type              VARCHAR2(32)    NOT NULL,           -- 事件类型（MESSAGE/SIGNAL/TIMER/ERROR）
    event_name              VARCHAR2(128)   NOT NULL,           -- 事件名称
    node_id                 VARCHAR2(128)   NOT NULL,           -- 节点ID
    state                   VARCHAR2(32)    NOT NULL DEFAULT 'ACTIVE', -- 状态（ACTIVE/DELETED）
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
...
CREATE TABLE wcdk_job (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    process_instance_id     VARCHAR2(64),                       -- 流程实例ID
    execution_id            VARCHAR2(64),                       -- 执行实例ID
    process_definition_id   VARCHAR2(64),                       -- 流程定义ID
    node_id                 VARCHAR2(128),                      -- 节点ID
    job_type                VARCHAR2(32)    NOT NULL,           -- 任务类型（ASYNC_CONTINUE/TIMER/SERVICE_TASK/EXTERNAL_TASK/HISTORY/OUTBOX_RETRY）
    status                  VARCHAR2(32)    NOT NULL DEFAULT 'READY', -- 状态（READY/CLAIMED/RETRY/DONE/DEAD/SUSPENDED）
    due_at                  TIMESTAMP,                          -- 到期时间
    retry_count             NUMBER(10)      NOT NULL DEFAULT 0, -- 重试次数
    max_retries             NUMBER(10)      NOT NULL DEFAULT 3, -- 最大重试次数
    lock_owner              VARCHAR2(128),                      -- 锁持有者
    lock_token              VARCHAR2(256),                      -- 锁令牌
    lock_until              TIMESTAMP,                          -- 锁到期时间
    payload                 CLOB,                               -- 任务负载（JSON）
    last_error              CLOB,                               -- 最后错误信息
    revision                NUMBER(19)      NOT NULL DEFAULT 1, -- 乐观锁版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_join_state (
    ...
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    process_instance_id     VARCHAR2(64)    NOT NULL,           -- 流程实例ID
    scope_execution_id      VARCHAR2(64)    NOT NULL,           -- 作用域执行实例ID
    gateway_id              VARCHAR2(128)   NOT NULL,           -- 网关节点ID
    cycle_key               VARCHAR2(128)   NOT NULL,           -- 循环KEY（区分多次汇合）
    expected_count          NUMBER(10)      NOT NULL,           -- 期望汇合数
    arrived_count           NUMBER(10)      NOT NULL DEFAULT 0, -- 已到达数
    status                  VARCHAR2(32)    NOT NULL DEFAULT 'WAITING', -- 状态（WAITING/COMPLETED）
    revision                NUMBER(19)      NOT NULL DEFAULT 1, -- 乐观锁版本号
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
    updated_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 更新时间
...
CREATE TABLE wcdk_outbox (
    id                      VARCHAR2(64)    NOT NULL,           -- 主键ID
    tenant_id               VARCHAR2(64)    NOT NULL DEFAULT 'default', -- 租户ID
    aggregate_type          VARCHAR2(64)    NOT NULL,           -- 聚合类型（PROCESS/TASK/JOB）
    aggregate_id            VARCHAR2(64)    NOT NULL,           -- 聚合ID
    event_type              VARCHAR2(64)    NOT NULL,           -- 事件类型
    payload                 CLOB            NOT NULL,           -- 事件负载（JSON）
    status                  VARCHAR2(32)    NOT NULL DEFAULT 'PENDING', -- 状态（PENDING/PROCESSED/FAILED）
    retry_count             NUMBER(10)      NOT NULL DEFAULT 0, -- 重试次数
    max_retries             NUMBER(10)      NOT NULL DEFAULT 3, -- 最大重试次数
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
...
CREATE TABLE wcdk_history_event (
    ...
    job_id                  VARCHAR2(64),                       -- 定时任务ID
    event_type              VARCHAR2(64)    NOT NULL,           -- 事件类型
    event_name              VARCHAR2(128),                      -- 事件名称
    node_id                 VARCHAR2(128),                      -- 节点ID
    user_id                 VARCHAR2(128),                      -- 用户ID
    payload                 CLOB,                               -- 事件负载（JSON）
    created_at              TIMESTAMP       NOT NULL DEFAULT SYSDATE, -- 创建时间
...
CREATE TABLE SYS_DEPT (
    ...
    DEPT_CODE               VARCHAR2(64)    NOT NULL,           -- 部门编码
    DEPT_NAME               VARCHAR2(128)   NOT NULL,           -- 部门名称
    SORT_NO                 NUMBER(10)      DEFAULT 0,          -- 排序号
    STATUS                  NUMBER(10)      DEFAULT 1,          -- 状态
...
CREATE TABLE SYS_PERMISSION (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    PARENT_ID               NUMBER(19),                         -- 上级权限ID
    PERMISSION_CODE         VARCHAR2(128)   NOT NULL,           -- 权限编码
    PERMISSION_NAME         VARCHAR2(128)   NOT NULL,           -- 权限名称
    PERMISSION_TYPE         VARCHAR2(32),                       -- 权限类型
    ROUTE_PATH              VARCHAR2(255),                      -- 路由地址
    SORT_NO                 NUMBER(10)      DEFAULT 0,          -- 排序号
    STATUS                  NUMBER(10)      DEFAULT 1,          -- 状态
    REMARK                  VARCHAR2(500),                      -- 备注
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
...
CREATE TABLE SYS_ROLE (
    ...
    ROLE_NAME               VARCHAR2(64)    NOT NULL,           -- 角色名称
    SORT_NO                 NUMBER(10)      DEFAULT 0,          -- 排序号
    STATUS                  NUMBER(10)      DEFAULT 1,          -- 状态
    REMARK                  VARCHAR2(500),                      -- 备注
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
...
CREATE TABLE SYS_USER (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    DEPT_ID                 NUMBER(19),                         -- 所属部门ID
    USERNAME                VARCHAR2(64)    NOT NULL,           -- 用户名
    PASSWORD_HASH           VARCHAR2(64)    NOT NULL,           -- 密码摘要
    REAL_NAME               VARCHAR2(64)    NOT NULL,           -- 姓名
    MOBILE                  VARCHAR2(32),                       -- 手机号
    EMAIL                   VARCHAR2(128),                      -- 邮箱
    STATUS                  NUMBER(10)      DEFAULT 1,          -- 状态
...
CREATE TABLE SYS_ROLE_PERMISSION (
    ROLE_ID                 NUMBER(19)      NOT NULL,           -- 角色ID
    PERMISSION_ID           NUMBER(19)      NOT NULL,           -- 权限ID
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
...
CREATE TABLE WCDK_PROCESS_CLIENT (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    CLIENT_ID               VARCHAR2(64)    NOT NULL,           -- 客户端标识
    CLIENT_NAME             VARCHAR2(128)   NOT NULL,           -- 客户端名称
    CALLBACK_URL            VARCHAR2(500),                      -- 回调地址
    AUTH_FLG                CLOB,                               -- 身份标识
    SERVICE_NAME            VARCHAR2(255),                      -- 服务名
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
    UPDATE_TIME             TIMESTAMP,                          -- 更新时间
...
CREATE TABLE WCDK_PROCESS_CLIENT_PROCESS (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    CLIENT_ID               VARCHAR2(64)    NOT NULL,           -- 客户端标识
    PROCESS_BEAN_NAME       VARCHAR2(128),                      -- 流程处理器名称
    PROCESS_DEFINITION_ID   VARCHAR2(128),                      -- 流程定义ID
    PROCESS_NAME            VARCHAR2(255),                      -- 流程名称
    EXCUTE_PARAM            VARCHAR2(1000),                     -- 执行参数
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
...
CREATE TABLE WCDK_PROCESS_DEFINITION_META (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    PROCESS_DEFINITION_ID   VARCHAR2(128)   NOT NULL,           -- Flowable流程定义ID
    PROCESS_DEFINITION_KEY  VARCHAR2(128),                      -- Flowable流程定义KEY
    PROCESS_DEFINITION_VERSION NUMBER(10),                      -- Flowable流程定义版本
    DEPLOYMENT_ID           VARCHAR2(128),                      -- Flowable部署ID
    INVALID_STATUS          NUMBER(10)      NOT NULL DEFAULT 0, -- 作废状态：0生效，1已作废
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
    UPDATE_TIME             TIMESTAMP,                          -- 更新时间
...
CREATE TABLE WCDK_PROCESS_FORM (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    FORM_KEY                VARCHAR2(128)   NOT NULL,           -- 表单标识
    FORM_NAME               VARCHAR2(255)   NOT NULL,           -- 表单名称
    FORM_VERSION            NUMBER(10)      NOT NULL DEFAULT 1, -- 表单版本
    FORM_SCHEMA_JSON        CLOB            NOT NULL,           -- 表单设计JSON
    RESOURCE_NAME           VARCHAR2(255),                      -- 表单资源名称
    TENANT_ID               VARCHAR2(128)   NOT NULL DEFAULT '', -- 租户ID
    STATUS                  NUMBER(10)      NOT NULL DEFAULT 1, -- 状态：1启用，0停用
    REMARK                  VARCHAR2(500),                      -- 备注
...
CREATE TABLE WCDK_PROCESS_FORM_BINDING (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    FORM_ID                 NUMBER(19)      NOT NULL,           -- 表单ID
    PROCESS_DEFINITION_ID   VARCHAR2(128),                      -- Flowable流程定义ID
    PROCESS_DEFINITION_KEY  VARCHAR2(128)   NOT NULL,           -- Flowable流程定义KEY
    PROCESS_DEFINITION_VERSION NUMBER(10),                      -- Flowable流程定义版本
    DEPLOYMENT_ID           VARCHAR2(128),                      -- Flowable部署ID
    TENANT_ID               VARCHAR2(128)   NOT NULL DEFAULT '', -- 租户ID
    BIND_SCOPE              VARCHAR2(32)    NOT NULL DEFAULT 'PROCESS', -- 绑定范围：PROCESS流程，START发起，TASK任务
    TASK_DEFINITION_KEY     VARCHAR2(128)   NOT NULL DEFAULT '', -- 任务定义KEY
    STATUS                  NUMBER(10)      NOT NULL DEFAULT 1, -- 状态：1启用，0停用
    REMARK                  VARCHAR2(500),                      -- 备注
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
    UPDATE_TIME             TIMESTAMP,                          -- 更新时间
...
CREATE TABLE WCDK_PROCESS_REQUEST (
    ID                      NUMBER(19)      NOT NULL,           -- 主键
    PROCESS_NO              VARCHAR2(64)    NOT NULL,           -- 流程申请编号
    STARTER                 VARCHAR2(64)    NOT NULL,           -- 流程发起人
    TASK_NAME               VARCHAR2(255)   NOT NULL,           -- 任务名称
    BUSINESS_TITLE          VARCHAR2(1000)  NOT NULL,           -- 业务标题
    FORM_DATA_JSON          CLOB,                               -- 动态表单数据JSON
    STATUS                  VARCHAR2(32)    NOT NULL,           -- 流程申请状态
    PROCESS_INSTANCE_ID     VARCHAR2(64),                       -- 流程实例ID
    CURRENT_TASK_ID         VARCHAR2(64),                       -- 当前任务ID
    CURRENT_TASK_NAME       VARCHAR2(128),                      -- 当前任务名称
    PROCESS_DEFINITION_KEY  VARCHAR2(128)   NOT NULL,           -- 流程定义标识
    CREATE_TIME             TIMESTAMP,                          -- 创建时间
    UPDATE_TIME             TIMESTAMP,                          -- 更新时间
...
-- 初始化部门数据
INSERT INTO SYS_DEPT VALUES (1001, NULL, 'HEAD', '总部', 1, 1, '默认根部门', SYSDATE, SYSDATE);

-- 初始化权限数据
INSERT INTO SYS_PERMISSION VALUES (1101, NULL, 'menu:home', '首页', 'MENU', '/home', 1, 1, '首页菜单权限', SYSDATE, SYSDATE, 'House');
INSERT INTO SYS_PERMISSION VALUES (1102, NULL, 'menu:deploy', '部署中心', 'MENU', '/deploy', 2, 1, '部署中心菜单权限', SYSDATE, SYSDATE, 'Upload');
INSERT INTO SYS_PERMISSION VALUES (1103, NULL, 'menu:model', '模型中心', 'MENU', '/model', 3, 1, '模型中心菜单权限', SYSDATE, SYSDATE, 'Collection');
INSERT INTO SYS_PERMISSION VALUES (1104, NULL, 'menu:designer', '流程设计', 'MENU', '/designer', 4, 1, '流程设计菜单权限', SYSDATE, SYSDATE, 'Edit');
INSERT INTO SYS_PERMISSION VALUES (1105, NULL, 'menu:process', '流程中心', 'MENU', '/process', 5, 1, '流程中心菜单权限', SYSDATE, SYSDATE, 'Operation');
INSERT INTO SYS_PERMISSION VALUES (1106, NULL, 'menu:task', '任务中心', 'MENU', '/task', 6, 1, '任务中心菜单权限', SYSDATE, SYSDATE, 'CircleCheck');
INSERT INTO SYS_PERMISSION VALUES (1107, NULL, 'menu:sys:user', '用户管理', 'MENU', '/system/user', 7, 1, '用户管理菜单权限', SYSDATE, SYSDATE, 'User');
INSERT INTO SYS_PERMISSION VALUES (1108, NULL, 'menu:sys:role', '角色管理', 'MENU', '/system/role', 8, 1, '角色管理菜单权限', SYSDATE, SYSDATE, 'Avatar');
INSERT INTO SYS_PERMISSION VALUES (1109, NULL, 'menu:sys:permission', '权限管理', 'MENU', '/system/permission', 9, 1, '权限管理菜单权限', SYSDATE, SYSDATE, 'Key');
INSERT INTO SYS_PERMISSION VALUES (1110, NULL, 'menu:sys:dept', '部门管理', 'MENU', '/system/dept', 10, 1, '部门管理菜单权限', SYSDATE, SYSDATE, 'OfficeBuilding');
INSERT INTO SYS_PERMISSION VALUES (1111, 1107, 'sys:user:view', '查看用户', 'BUTTON', NULL, 11, 1, '用户查询权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1112, 1107, 'sys:user:add', '新增用户', 'BUTTON', NULL, 12, 1, '用户新增权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1113, 1107, 'sys:user:edit', '修改用户', 'BUTTON', NULL, 13, 1, '用户修改权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1114, 1107, 'sys:user:delete', '删除用户', 'BUTTON', NULL, 14, 1, '用户删除权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1115, 1108, 'sys:role:view', '查看角色', 'BUTTON', NULL, 15, 1, '角色查询权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1116, 1108, 'sys:role:add', '新增角色', 'BUTTON', NULL, 16, 1, '角色新增权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1117, 1108, 'sys:role:edit', '修改角色', 'BUTTON', NULL, 17, 1, '角色修改权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1118, 1108, 'sys:role:delete', '删除角色', 'BUTTON', NULL, 18, 1, '角色删除权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1119, 1109, 'sys:permission:view', '查看权限', 'BUTTON', NULL, 19, 1, '权限查询权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1120, 1109, 'sys:permission:add', '新增权限', 'BUTTON', NULL, 20, 1, '权限新增权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1121, 1109, 'sys:permission:edit', '修改权限', 'BUTTON', NULL, 21, 1, '权限修改权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1122, 1109, 'sys:permission:delete', '删除权限', 'BUTTON', NULL, 22, 1, '权限删除权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1123, 1110, 'sys:dept:view', '查看部门', 'BUTTON', NULL, 23, 1, '部门查询权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1124, 1110, 'sys:dept:add', '新增部门', 'BUTTON', NULL, 24, 1, '部门新增权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1125, 1110, 'sys:dept:edit', '修改部门', 'BUTTON', NULL, 25, 1, '部门修改权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1126, 1110, 'sys:dept:delete', '删除部门', 'BUTTON', NULL, 26, 1, '部门删除权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1127, NULL, '*:*:*', '系统全量权限', 'BUTTON', NULL, 99, 1, '管理员全量权限', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1128, NULL, 'menu:form', '表单设计', 'MENU', '/form-designer', 5, 1, '表单设计菜单权限', SYSDATE, SYSDATE, 'Tickets');
INSERT INTO SYS_PERMISSION VALUES (1130, 1102, 'deploy:tab:create', '创建部署', 'TAB', NULL, 1, 1, '部署中心创建部署', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1131, 1102, 'deploy:tab:list', '流程列表', 'TAB', NULL, 2, 1, '部署中心流程列表', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1132, 1103, 'model:tab:create', '创建模型', 'TAB', NULL, 1, 1, '模型中心创建模型', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1133, 1103, 'model:tab:list', '模型列表', 'TAB', NULL, 2, 1, '模型中心模型列表', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1134, 1105, 'process:tab:create', '创建流程', 'TAB', NULL, 1, 1, '流程中心创建流程', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1135, 1105, 'process:tab:list', '流程列表', 'TAB', NULL, 2, 1, '流程中心流程列表', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1136, 1102, 'deploy:refresh', '刷新部署', 'BUTTON', NULL, 10, 1, '刷新部署中心数据', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1137, 1102, 'deploy:create', '提交部署', 'BUTTON', NULL, 11, 1, '提交流程部署', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1138, 1102, 'deploy:start', '发起审批', 'BUTTON', NULL, 12, 1, '从部署发起审批', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1139, 1102, 'deploy:edit', '编辑部署', 'BUTTON', NULL, 13, 1, '编辑部署流程定义', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1140, 1102, 'deploy:view', '查看部署', 'BUTTON', NULL, 14, 1, '查看部署详情', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1141, 1102, 'deploy:preview', '预览部署', 'BUTTON', NULL, 15, 1, '预览部署流程图', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1142, 1102, 'deploy:binding', '修改部署绑定', 'BUTTON', NULL, 16, 1, '修改部署客户端绑定', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1143, 1102, 'deploy:delete', '删除部署', 'BUTTON', NULL, 17, 1, '删除部署数据', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1144, 1103, 'model:refresh', '刷新模型', 'BUTTON', NULL, 10, 1, '刷新模型中心数据', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1145, 1103, 'model:create', '创建模型', 'BUTTON', NULL, 11, 1, '创建流程模型', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1146, 1103, 'model:edit', '修改模型', 'BUTTON', NULL, 12, 1, '修改流程模型', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1147, 1103, 'model:preview', '预览模型', 'BUTTON', NULL, 13, 1, '预览流程模型', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1148, 1103, 'model:deploy', '部署模型', 'BUTTON', NULL, 14, 1, '部署流程模型', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1149, 1103, 'model:delete', '删除模型', 'BUTTON', NULL, 15, 1, '删除流程模型', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1150, 1105, 'process:refresh', '刷新流程', 'BUTTON', NULL, 10, 1, '刷新流程中心数据', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1151, 1105, 'process:view', '查看流程', 'BUTTON', NULL, 11, 1, '查看流程详情', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1152, 1105, 'process:submit', '提交流程', 'BUTTON', NULL, 12, 1, '提交草稿流程', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1153, 1105, 'process:delete', '删除流程', 'BUTTON', NULL, 13, 1, '删除流程申请', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1154, 1105, 'process:action:draft', '保存草稿', 'BUTTON', NULL, 14, 1, '流程表单保存草稿按钮', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1155, 1105, 'process:action:submit', '提交申请', 'BUTTON', NULL, 15, 1, '流程表单提交申请按钮', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1156, 1105, 'process:action:reset', '重置流程表单', 'BUTTON', NULL, 16, 1, '流程表单重置按钮', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1157, 1106, 'task:refresh', '刷新任务', 'BUTTON', NULL, 10, 1, '刷新任务中心数据', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1158, 1106, 'task:view', '查看任务', 'BUTTON', NULL, 11, 1, '查看任务流程图', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1159, 1106, 'task:approve', '办理任务', 'BUTTON', NULL, 12, 1, '办理审批任务', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1160, 1106, 'task:delete', '删除任务', 'BUTTON', NULL, 13, 1, '删除任务', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1163, 1105, 'process:status:tag', '流程状态', 'TAG', NULL, 30, 1, '流程中心状态资源', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1164, 1104, 'designer:export:bpmn', '导出 BPMN', 'BUTTON', NULL, 10, 1, '导出 BPMN 文件', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1165, 1104, 'designer:export:bpmn-xml', '导出 BPMN.XML', 'BUTTON', NULL, 11, 1, '导出 BPMN.XML 文件', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1166, 1104, 'designer:export:png', '导出 PNG', 'BUTTON', NULL, 12, 1, '导出 PNG 图片', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1167, 1104, 'designer:canvas:center', '居中显示', 'BUTTON', NULL, 13, 1, '将画布内容居中显示', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1168, 1104, 'designer:canvas:reset', '清空画布', 'BUTTON', NULL, 14, 1, '清空画布内容', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1169, 1104, 'designer:canvas:refresh', '刷新画布', 'BUTTON', NULL, 15, 1, '刷新画布内容', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1170, 1128, 'form:save', '保存表单方案', 'BUTTON', NULL, 10, 1, '保存表单设计方案', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1171, 1128, 'form:load', '载入表单方案', 'BUTTON', NULL, 11, 1, '载入表单设计方案', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1172, 1128, 'form:export', '导出表单JSON', 'BUTTON', NULL, 12, 1, '导出表单设计JSON', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1173, 1128, 'form:import', '导入表单JSON', 'BUTTON', NULL, 13, 1, '导入表单设计JSON', SYSDATE, SYSDATE, NULL);
INSERT INTO SYS_PERMISSION VALUES (1174, 1128, 'form:jump:designer', '跳转流程设计', 'BUTTON', NULL, 14, 1, '从表单设计跳转流程设计', SYSDATE, SYSDATE, NULL);

-- 初始化角色数据
INSERT INTO SYS_ROLE VALUES (1001, 'ADMIN', '系统管理员', 1, 1, '默认管理员角色', SYSDATE, SYSDATE);

-- 初始化用户数据（默认密码: admin123）
INSERT INTO SYS_USER VALUES (1001, 1001, 'admin', '0192023A7BBD73250516F069DF18B500', '系统管理员', NULL, NULL, 1, NULL, SYSDATE, SYSDATE);

-- 初始化用户角色关联数据
INSERT INTO SYS_USER_ROLE VALUES (1001, 1001, SYSDATE);

-- 初始化角色权限关联数据（管理员拥有所有权限）
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1101, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1102, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1130, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1131, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1136, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1137, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1138, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1139, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1140, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1141, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1142, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1143, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1103, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1132, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1133, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1144, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1145, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1146, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1147, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1148, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1149, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1104, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1164, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1165, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1166, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1167, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1168, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1169, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1105, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1134, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1135, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1150, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1151, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1152, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1153, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1154, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1155, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1156, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1163, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1106, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1157, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1158, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1159, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1160, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1107, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1111, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1112, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1113, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1114, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1108, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1115, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1116, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1117, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1118, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1109, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1119, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1120, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1121, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1122, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1110, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1123, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1124, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1125, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1126, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1127, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1170, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1171, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1172, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1173, SYSDATE);
INSERT INTO SYS_ROLE_PERMISSION VALUES (1001, 1174, SYSDATE);
